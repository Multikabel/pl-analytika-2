...
class RefereeTests(unittest.TestCase):
    ...
    def test_styling_same_column_unrounded_and_missing_neutral(self):
        ...
        self.assertIn("#a92732", styles.loc[0, "F D"])
        self.assertIn("#2f7d3f", styles.loc[1, "F D"])
        self.assertIn("; color: #f5f5f5", styles.loc[0, "F D"])
        self.assertIn("; color: #f5f5f5", styles.loc[1, "F D"])
        self.assertEqual(styles.loc[2, "F D"], "")
        self.assertTrue(styles["F H"].str.contains("#2f7d3f").all())
        self.assertTrue(styles["F H"].str.contains("; color: #f5f5f5", regex=False).all())
